#!/usr/bin/env python

"""
An echo server that uses threads to handle multiple clients at a time.
Entering any line of input at the terminal will exit the server.
"""

import select
import socket
import sys
import threading

#instead of globals maybe make a class
global ball1check, ball2check

class Server:
    def __init__(self):
        self.host = ''
        self.port = 50000
        self.backlog = 5
        self.size = 1024
        self.server = None
        self.threads = []

    def open_socket(self):
        try:
            self.server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.server.bind((self.host,self.port))
            self.server.listen(5)
        except socket.error, (value,message):
            if self.server:
                self.server.close()
            print "Could not open socket: " + message
            sys.exit(1)

    def run(self):
        self.open_socket()
        input = [self.server,sys.stdin]
        running = 1
        while running:
            inputready,outputready,exceptready = select.select(input,[],[])
            print "server running"
            for s in inputready:
                print "someone is joining"
                if s == self.server:
                    print "if s==self.server"
                    # handle the server socket
                    c = Client(self.server.accept())
                    print 'Connected with ' + str(c.client) + ':' + str(c.address) + "\n"
                    c.set_server(self)
                    c.start()
                    self.threads.append(c)
                    print self.threads
                    print "\n"

                elif s == sys.stdin:
                    # handle standard input
                    junk = sys.stdin.readline()
                    running = 0 
            print "after for block"

        # close all threads
        print "before close"
        self.server.close()
        for c in self.threads:
            print c.client
            print c.address
            c.join()
    
    def hole_in_one(self):
        print " ()()()()()()()()()() \n"
        print " \n HOLE IN ONE HAPPENED \n"
        print " telling all clients to do their blinky \n"
        print " ()()()()()()()()()() \n"
        print self.threads
        print "threads above\n"
        for c in self.threads:
            c.blinky()
            print "client name and address: \n"
            print c.client
            print c.address
            c.client.send("0=hole_in_one")

class Client(threading.Thread):
    def __init__(self,(client,address)):
        threading.Thread.__init__(self)
        self.client = client
        self.address = address
        self.size = 1024
        
    def blinky(self):
        print "A=hole-in-one \n"
        print "********************** \n"
        print "BLINKY BLINKY BLINKY WE GOT A HOLE IN ONE YAAAAAAYYYYYY!!!! \n"
        print "********************** \n"

    def set_server(self, somevariable):
        self.server = somevariable
        print "\n----------------=-=-=-=-=-\n"
        print self.server
        print "\n----------------=-=-=-=-=-\n"

    def run(self):
        running = 1
        while running:
            data = self.client.recv(self.size)
            if data:
                global ball1check, ball2check
                lhs, rhs = data.split("=", 1)
                rhs = rhs[:-1]
                sys.stdout.write(data)
                if lhs == "1":
                    self.client.send("1=got_message")
                    if rhs == "ball":
                        ball1check = 1
                        #triggering a pole resets all poles after it
                        ball2check = 0
                        self.client.send("\n 1=ball one check is true\n")
                        self.client.send("\n 1=ball1check\n"+str(ball1check))
                        self.client.send("\n 1=ball2check\n"+str(ball2check))
                        self.client.send("\n 1=\n")
                        sys.stdout.write("\n ball1check\n"+str(ball1check))
                        sys.stdout.write("\n ball2check\n"+str(ball2check))
                        sys.stdout.write("\n\n")
        
                elif lhs == "2":
                    self.client.send("2=got_message")

                    if rhs == "ball":
                        ball2check = 1
                        self.client.send("\n 2=ball two check is true\n")
                        self.client.send("\n 2=ball1check\n"+str(ball1check))
                        self.client.send("\n 2=ball2check\n"+str(ball2check))
                        self.client.send("\n 2=\n")
                        if ball1check == 1:
                            self.client.send("\n 2=one and two in succession - hole in one - resetting checks\n")
                            self.server.hole_in_one()
                            sys.stdout.write("\n HOLE IN ONE \n"+str(ball1check))
                            ball1check = 0
                            ball2check = 0
                        else:
                            self.client.send("\n 2=no hole in one - resetting checks\n ")
                            sys.stdout.write("\n 2=RESET \n"+str(ball1check))
                            ball1check = 0
                            ball2check = 0
                        sys.stdout.write("\n 2=ball1check\n"+str(ball1check))
                        sys.stdout.write("\n 2=ball2check\n"+str(ball2check))
                        sys.stdout.write("\n 2=\n")
        
                else:
                    self.client.send(lhs)
                    self.client.send("\n you are not a number\n")
                    self.client.send("\n message ignored\n")
                    self.client.send(rhs)
                    self.client.send("\n\n")
                    self.client.send("----------------------")
                    self.client.send("\n\n")

            else:
                self.client.close()
                running = 0

if __name__ == "__main__":
    ball1check = 0
    ball2check = 0
    s = Server()
    s.run()
